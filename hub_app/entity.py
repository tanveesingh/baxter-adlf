...
'''16-Feb-2021: skip logic added for read_glue operation in case of 0 count from the dataframe '''
class Entity:
    ...
    def run_last_ops(self,**op_tags):
        try:
            if 'run_ops' in op_tags:
                ...
                elif 'etctl' in self.db_name:
                    self.folder_name = 'ETCTL'
                ...
                if status:
                    for op_id in l_v:
                        if op_id in self.d_ops:
                            ...
                            if res and str(error) == 'skip':
                                status = False
                                log.warn("Skipping the Pipeline")
                                return True, "skip"
    ...
    def run(self):
        ...
        if error == "Please make an entry to Operation/Job Config Table":
            ...
            return False
                ...
                abc.update_abc_skipping(self.etl_prcs_run_id,self.etl_prcs_id,self.prcs_name)     
                return True    
        else:
