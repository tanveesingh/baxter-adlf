        ...
        if len(res_set_param) == 1:
            ...
            if LoadType == 'FULL' or cdc_lower_dtm == '1950-12-31 00:00:00':
                ...
                if len(files_list) > 0:
                    ...
                    sql = "select to_char(max(cdc_low_dtm),'YYYY-MM-DD HH24:MI:SS'), to_char(max(cdc_upr_dtm),'YYYY-MM-DD HH24:MI:SS') from etl_prcs_run where etl_prcs_id = (select etl_prcs_id from etl_prcs where etl_prcs_name = '" + prcs_name + "' and rgn_cd = '" + region + "') and RUN_STS_CD = 1"
                    cursor.execute(sql)
                    res_set = cursor.fetchall()
                    cdc_lower_dtm = res_set[0][0]
                    if (cdc_lower_dtm is None) or (cdc_lower_dtm == ''):
