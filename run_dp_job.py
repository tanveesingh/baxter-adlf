...
## 16-Feb-2021: Update: when skip pipeline, log cdc_low_dtm as max(cdc_upr_dtm) from successful runs
def main():
    try:
        ct_dt = datetime.datetime.now()
        ...
        jp_table_sql = "select parm_val from etl_prcs_parm epp where parm_name = 'jp_TableName' and etl_prcs_id = " + etl_prcs_id + " and actv_ind = 'Y'"
        connection = get_rds_con(hub)
        cursor = connection.cursor()
        cursor.execute(jp_table_sql)
        res_set_param = cursor.fetchall()
        ...
        cursor.execute(sql)
        res_set_glue = cursor.fetchall()     
        if len(res_set_param) == 1:
            table_name = res_set_param[0][0]
            ...
            if LoadType == 'FULL' or cdc_lower_dtm == '1950-12-31 00:00:00':
                ...
                if 'ladta' in res_set[0][0]:
                    ...
                    db_name = 'APDD'
                elif 'eudd' in res_set[0][0]:
                    db_name = 'EUDD'
                elif 'eudta' in res_set[0][0]:
                    db_name = 'EUDTA'
                elif 'euctl' in res_set[0][0]:
                    db_name = 'EUCTL'
                elif 'etdta' in res_set[0][0]:
                    db_name = 'ETDTA'
                elif 'etctl' in res_set[0][0]:
                    db_name = 'ETCTL'
                ...
                if len(files_list) > 0:
                    ...
                    # 16-Feb-2021: Change added 
                    sql = "select to_char(max(cdc_upr_dtm),'YYYY-MM-DD HH24:MI:SS'), to_char(max(cdc_upr_dtm),'YYYY-MM-DD HH24:MI:SS') from etl_prcs_run where etl_prcs_id = (select etl_prcs_id from etl_prcs where etl_prcs_name = '" + prcs_name + "' and rgn_cd = '" + region + "') and RUN_STS_CD = 1"
                    cursor.execute(sql)
                    res_set = cursor.fetchall()
                    cdc_lower_dtm = res_set[0][0]
                    if (cdc_lower_dtm is None) or (cdc_lower_dtm == ''):
                        cdc_lower_dtm = '1950-12-31 00:00:00'
                    cdc_upper_dtm = res_set[0][1]
                    if (cdc_upper_dtm is None) or (cdc_upper_dtm == ''):
                    ...
                    #close_rds_con(connection)
        elif len(res_set_glue)==1:
            table_name = res_set_glue[0][0]
            print("Table_Name: ",table_name)
            etl_prcs_id, error = get_prcs_id(prcs_name, region)
            sql = "select parm_val from etl_prcs_parm epp where parm_name = 'jp_LoadType' and etl_prcs_id = " + etl_prcs_id + " and actv_ind = 'Y'"
            cursor.execute(sql)
...
def trigger_command():
    try:
        ...
        if len(arg_list) == 7:
            ...
            #repurposing o_date = arg_list[8] and reporting_code = arg_list[9] to add additional spark configurations from Control-M
            if(o_date == 'add_configs'):
                
                add_conf_list = reporting_code.split(',')
                for val in add_conf_list:
                    if val == 'conf_crossJoin':
                        add_configs = add_configs + ' --conf spark.sql.crossJoin.enabled=true'
                        #add / append more configs here
        ...
        cmd = """spark-submit --packages net.snowflake:snowflake-jdbc:3.8.4,net.snowflake:spark-snowflake_2.11:2.5.0-spark_2.4 --jars /usr/lib/spark/jars/httpclient-4.5.9.jar,/usr/lib/hudi/hudi-spark-bundle.jar {add_configs} --deploy-mode client --master yarn --py-files /home/hadoop/Framework/Data_Processing/SparkHub/hub_config.py /home/hadoop/Framework/Data_Processing/SparkHub/run.py {application_args}""".format(add_configs = add_configs, application_args = application_args)
        cmd = cmd.strip()
        exitcode = None
        print(cmd)
        cmd_args = re.split('[\r\n\t ]+', cmd)
    ...
    try:
        with connection.cursor() as cursor:
            ...
            sql = "INSERT INTO ETL_PRCS_RUN(ETL_PRCS_RUN_ID,ETL_PRCS_ID,ETL_SCHD_RUN_ID,ETL_BTCH_RUN_ID,ETL_SBJCT_AREA_RUN_ID,STRT_DTM,RUN_STS_CD,RUN_STS_DESCR," \
                      ...
                      "(" + etl_prcs_run_id + "," + etl_prcs_id + "," + etl_schd_run_id + "," + etl_btch_run_id + "," + etl_sbjct_area_run_id + ",'" + ct_dt_str + "',1," \
                      "'Finished','" + sig_file_name + "','" + ct_dt_str +"','"+ ct_dt_str +"','"+ ct_dt_str +"','emruser','emruser','"+cdc_lower_dtm+"','"+cdc_upper_dtm+"')"
            cursor.execute(sql)
            connection.commit()
            close_rds_con(connection)
    ...
    try:
        with connection.cursor() as cursor:
            sql = "INSERT INTO ETL_JOB_RUN(ETL_JOB_RUN_ID,ETL_JOB_ID,ETL_PRCS_RUN_ID,PARTN_VAL,STRT_DTM,RUN_STS_CD,RUN_STS_DESCR," \
                      ...
                      "(" + etl_job_run_id + "," + etl_job_id + "," + etl_prcs_run_id + ",'" + 'Non-Partitioned' + "','" + ct_dt_str + "',1," \
                      "'Finished','" + ct_dt_str +"','"+ ct_dt_str +"','"+ ct_dt_str + "','emruser','emruser')"
            cursor.execute(sql)
            connection.commit()
